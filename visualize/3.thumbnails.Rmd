...
load_data %>%
  transmute(
    ...
    well = sprintf("r%02dc%02d", Metadata_Row, Metadata_Col),
    basedir = basedir,
    cachedir = cachedir,
    montagedir = montagedir
  ) %>%
...
EXPERIMENT_TAG=Standard_Compound_U2OS_48
...
parallel \
  --header ".*\n" \
  -C "," \
  --joblog ${IMAGE_DIR}/create_montage.log \
...
parallel \
  -a /tmp/plates.txt \
  --no-run-if-empty \
  --joblog ${DEST_IMAGE_DIR}/create_thumbnail.log \
