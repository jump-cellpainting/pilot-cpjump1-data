...
metric_sets[["level_1_metrics"]] %>%
  group_by(Metadata_experiment_condition, Metadata_experiment_type, Metadata_timepoint, Metadata_pert_or_negcon) %>%
  tally()
```

```{r}
metric_sets[["level_2_1_metrics"]] %>%
  group_by(Metadata_experiment_condition, Metadata_experiment_type, Metadata_timepoint, Metadata_pert_or_negcon) %>%
  tally()
```
...
y_threshold <-
  level_1_metrics %>%
  filter(Metadata_pert_or_negcon == "negcon") %>%
  group_by(Metadata_experiment_type,
           Metadata_timepoint) %>%
  summarise(
    sim_scaled_mean_ref_i_mean_i_thresh =
      mean(sim_scaled_mean_ref_i_mean_i) + 3 * sd(sim_scaled_mean_ref_i_mean_i),
    .groups = "keep"
  )
...
p <- 
  level_1_metrics %>%
  mutate(point_order = as.numeric(factor(Metadata_pert_or_negcon, levels = c("empty", "negcon", "pert"), ordered = TRUE))) %>%
  arrange(desc(point_order)) %>%
  ggplot(aes(cell_count, 
             sim_scaled_mean_ref_i_mean_i, 
             color = Metadata_pert_or_negcon)) + 
  geom_point() + 
  scale_colour_manual(values = c("green", "red", "black")) + 
  facet_wrap(Metadata_experiment_type~Metadata_timepoint, ncol = 2) + 
  geom_hline(data = y_threshold, aes(yintercept = sim_scaled_mean_ref_i_mean_i_thresh), alpha = 0.5) +
  geom_vline(data = x_threshold, aes(xintercept = cell_count_thresh), alpha = 0.5) +
  ggtitle(experiment_tag)
...
y_threshold <-
  ...
  group_by(Metadata_experiment_type,
           Metadata_timepoint) %>%
  summarise(
    sim_mean_i_mean_i_thresh =
      mean(sim_mean_i_mean_i) + 3 * sd(sim_mean_i_mean_i),
    .groups = "keep"
  )
...
p <- 
  ...
  scale_colour_manual(values = c("green", "red", "black")) + 
  facet_wrap(Metadata_experiment_type~Metadata_timepoint, ncol = 2) + 
  geom_hline(data = y_threshold, aes(yintercept = sim_mean_i_mean_i_thresh), alpha = 0.5) +
  geom_vline(data = x_threshold, aes(xintercept = cell_count_thresh), alpha = 0.5) +
  ggtitle(experiment_tag)
